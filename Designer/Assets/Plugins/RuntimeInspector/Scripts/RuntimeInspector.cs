...
namespace RuntimeInspectorNamespace
{
	...
	{
		public BoundInputField.OnValueChangedDelegate InspectorValueChangedEvent;
		...
		public List<string> notShowObjectsName = new List<string>();
		public List<GameObject> notshowObjects = new List<GameObject>();
...
#pragma warning restore 0649
		...
		private void Initialize()
		{
			...
			for (int i = 0; i < notShowObjectsName.Count; i++)
			{
				GameObject notShowObj = GameObject.Find(notShowObjectsName[i]);
				
				if(notShowObj)
					RuntimeInspectorUtils.IgnoredTransformsInHierarchy.Add(notShowObj.transform);
			}
			
			for (int i = 0; i < notshowObjects.Count; i++)
			{
				RuntimeInspectorUtils.IgnoredTransformsInHierarchy.Add(notshowObjects[i].transform);
			}
		...
		public void Refresh()
		{
			if( IsBound )
			{
				...
				{
					// 맨 처음 Inspector 창에서 Position 값 수정 시 선택한 칸이 해제되고, 바로 Scale로 넘어가버림
					// 역으로 Scale 값 수정 시 선택한 칸이 해제되고, 바로 Position으로 넘어가는 오류 있음
					// 따라서, 두번 해당 함수를 호출하여 해결
		...
		{
			nextRefreshTime = Time.realtimeSinceStartup + m_refreshInterval;;
			// nextRefreshTime = 0f;
		}
		...
		private InspectorField InstantiateDrawer( InspectorField drawer, Transform drawerParent )
		{
			...
			if (newDrawer.TryGetComponent(out Vector3Field field))
			{
				field.SetAdditionalEventForInputfields(InspectorValueChangedEvent);
			}
